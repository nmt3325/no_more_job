"""バイトルのツール定義（統合サーバーに登録する）"""

from typing import Optional

from . import baitoru_api as api


def _format_jobs(jobs_raw: list) -> list:
    return [
        {
            "source": "バイトル",
            "job_id": j.get("id", ""),
...
def register(mcp) -> None:
    ...
    def search(
        ...
        return {
            ...
            "count": len(jobs_raw),
            "search_url": url,
            "jobs": _format_jobs(jobs_raw),
        }

    @mcp.tool(name="baitoru_search_station")
    def search_station(station_name: str, region: str = "kanto") -> list:
        """駅名でバイトルの駅コードを検索する。
        baitoru_search() の eki_codes に使う値を返す。
        ...
        Args:
            ...
            region: 地域コード (baitoru_search() と同じ値)
    ...
    @mcp.tool(name="baitoru_search_with_station")
