...
from . import townwork_api as api

# ── Playwright ブラウザの管理 ─────────────────────────────────
# サーバープロセス起動中は1つのブラウザを保持して使い回す。
_playwright = None
...
def register(mcp) -> None:
    ...
    @mcp.tool(name="townwork_search_with_station")
    ...
    @mcp.tool(name="townwork_search_station")
    async def search_station(station_name: str, prefecture: str = "東京") -> list:
        """駅名でタウンワークの駅を検索する。
        通常は townwork_search_with_station() で一括実行できるが、
        駅候補を先に確認したい場合に使う。
    ...
    @mcp.tool(name="townwork_get_job_count")
    async def get_job_count(keyword: str, prefecture: str = "東京") -> dict:
        """タウンワークでキーワード検索のヒット件数を取得する。
