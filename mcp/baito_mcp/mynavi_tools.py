"""マイナビバイトのツール定義（統合サーバーに登録する）"""

from typing import Optional

from . import mynavi_api as api
...
def register(mcp) -> None:
    ...
    def search(
        ...
        Args:
            ...
            kodawari_ids: こだわり条件IDリスト (mynavi_get_filters() で確認可)
                代表的なID: 1=駅徒歩5分 | 6=日払い週払い | 9=未経験歓迎 | 23=シフト自由 | 39=交通費支給
                42=大学生歓迎 | 45=主婦(夫)歓迎 | 48=副業Wワーク | 68=完全週休2日 | 87=入社祝い金
            route_ids: 路線/駅IDリスト (mynavi_search_station() が返す route_id を渡す。例: ["13-6-25"])
    ...
    @mcp.tool(name="mynavi_get_detail")
    def get_detail(job_id: str) -> dict:
        """マイナビバイトの求人詳細を取得する。

        Args:
            job_id: 求人コード (例: "J0134106676") - mynavi_search() の結果の job_id フィールド
        """
    ...
    @mcp.tool(name="mynavi_get_filters")
    def get_filters() -> dict:
        """マイナビバイトの利用可能フィルター一覧（こだわり条件・雇用形態・勤務期間など）を返す。
        mynavi_search() の kodawari_ids に渡すIDを調べるときに使う。
        """
